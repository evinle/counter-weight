...
function makeTimer(overrides: Partial<Timer> = {}): Timer {
  const rightAfter = new Date(Date.now() + 5000);
  return {
    id: 1,
    title: "Test",
    description: null,
    emoji: null,
    targetDatetime: rightAfter,
...
describe("timerStore", () => {
  ...
  it("sets firedTimer when a timer fires", () => {
    const timer = makeTimer({
      id: 1,
      targetDatetime: new Date(Date.now() + 1000),
    });
  ...
  it("reschedules when a sooner timer is added", () => {
    const later = makeTimer({
      id: 1,
      targetDatetime: new Date(Date.now() + 5000),
    });
    const sooner = makeTimer({
      id: 2,
      targetDatetime: new Date(Date.now() + 1000),
    });
  ...
  it("fires multiple timers in order", () => {
    ...
    const first = makeTimer({
      id: 1,
      targetDatetime: new Date(Date.now() + 1000),
    });
    const second = makeTimer({
      id: 2,
      targetDatetime: new Date(Date.now() + 2000),
    });
  ...
  it("dismissFired clears firedTimer", () => {
    const timer = makeTimer({
      id: 1,
      targetDatetime: new Date(Date.now() + 100),
    });
    useTimerStore.getState().sync([timer]);
