...
describe('durationToMs', () => {
  ...
  it('handles hours only', () => {
    expect(durationToMs(0, 2, 0)).toBe(7_200_000)
  })
})

describe('msToDuration', () => {
  it('converts ms to days hours minutes', () => {
    expect(msToDuration((86400 + 7200 + 1800) * 1000)).toEqual({
  ...
  it('truncates sub-minute ms', () => {
    expect(msToDuration(90_000)).toEqual({ days: 0, hours: 0, minutes: 1 })
  })

  it('round-trips through durationToMs', () => {
    const original = { days: 2, hours: 3, minutes: 45 }
    expect(
      msToDuration(durationToMs(original.days, original.hours, original.minutes))
    ).toEqual(original)
  })
})
