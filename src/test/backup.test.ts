...
describe("exportTimers", () => {
  ...
  it("serialises dates as ISO strings", () => {
    ...
    expect(new Date(t.targetDatetime).toISOString()).toBe(
...
describe("importTimers", () => {
  ...
  it("handles unknown version as best-effort with handleDefault", () => {
    const envelope = JSON.stringify({
      ...
      timers: [
        makeTimer({ targetDatetime: new Date("2026-06-01T10:00:00.000Z") }),
      ].map((t) => ({
        ...t,
      ...
      timers: [
        {
          title: "Valid",
          status: "active",
          targetDatetime: "2026-06-01T10:00:00.000Z",
          priority: "medium",
        },
        { title: "No status" },
        { status: "active" },
  ...
  it("throws when version field is missing", () => {
    expect(() => importTimers(JSON.stringify({ timers: [] }))).toThrow(
      "Missing version field",
    );
  });

  it("throws when timers array is missing", () => {
    expect(() => importTimers(JSON.stringify({ version: 1 }))).toThrow(
      "Missing timers array",
    );
  });
});
