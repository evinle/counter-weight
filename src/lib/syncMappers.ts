...
export function mapServerGroup(
  ...
  return {
    ...
    emoji: s.emoji,
    color: s.color,
    conditions: s.conditions as GroupConditions,
    version: s.version,
    createdAt: new Date(s.createdAt),
    updatedAt: new Date(s.updatedAt),
    syncStatus: SyncStatuses.Synced,
...
export function mapServerTimer(
  ...
  return {
    ...
    emoji: s.emoji,
    targetDatetime: new Date(s.targetDatetime as string),
    originalTargetDatetime: new Date(s.originalTargetDatetime as string),
    status: s.status,
    priority: s.priority,
    recurrenceRule: s.recurrenceRule as { cron: string; tz: string } | null,
    version: s.version,
    tagIds: s.tagIds as string[],
