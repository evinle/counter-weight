...
beforeEach(() => {
  ...
  process.env.AUTH_CALLBACK_URL_LOCAL = "https://localhost:5174/auth/callback";
});
...
async function getClientSecret(): Promise<string> {
  ...
  const { SecretString } = await sm.send(
    new GetSecretValueCommand({
      SecretId: process.env.COGNITO_CLIENT_SECRET_ARN!,
    }),
  );
...
export const authRoutes: FastifyPluginAsync = async (app) => {
  app.post("/callback", async (req, reply) => {
    ...
    const ALLOWED_ORIGINS: Record<string, string | undefined> = {
      "https://localhost:5174": process.env.AUTH_CALLBACK_URL_LOCAL,
