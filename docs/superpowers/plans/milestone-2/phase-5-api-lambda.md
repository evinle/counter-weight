...
function makeCtx(userId: string | null, dbValues?: any[]) {
  ...
  const mockValues = vi
    .fn()
    .mockReturnValue({ onConflictDoUpdate: vi.fn().mockResolvedValue([]) });
...
export const timersRouter = router({
  list: protectedProcedure.query(async ({ ctx }) => {
    return ctx.db
      ...
      .from(timers)
      .where(
        and(eq(timers.userId, ctx.userId), ne(timers.status, "cancelled")),
      );
  }),
  ...
  reconcile: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input.since)
        conditions.push(gt(timers.updatedAt, new Date(input.since)));
